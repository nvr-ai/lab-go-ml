...
func BenchmarkIoU_RandomPairs(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_ = CalculateIoU(pair.r1, pair.r2)
	}
}
...
func BenchmarkIoU_RealImageBounds(b *testing.B) {
	// Load a subset of real images to get realistic bounds
	images, err := util.LoadDirectoryImageFiles("../../../ml/corpus/images/clip-4k.mp4")
	if err != nil {
	...
	testImages := images[:min(50, len(images))]
	...
	for i := range testImages {
		...
		for j := 0; j < 10; j++ { // Generate 10 pairs per image
			...
			minSize := int(float64(min(imgWidth, imgHeight)) * 0.05)
			maxSize := int(float64(min(imgWidth, imgHeight)) * 0.25)
			...
			if rand.Float32() < 0.7 {
				...
				x2 = max(0, min(imgWidth-w2, x1+offsetX))
				y2 = max(0, min(imgHeight-h2, y1+offsetY))
			} else {
				// Random placement
			...
			rectanglePairs[idx].r1 = Rect{X1: x1, Y1: y1, X2: x1 + w1, Y2: y1 + h1}
			rectanglePairs[idx].r2 = Rect{X1: x2, Y1: y2, X2: x2 + w2, Y2: y2 + h2}
		}
	}
...
func BenchmarkIoU_BatchProcessing(b *testing.B) {
	...
	for i := range detections {
		...
		centerX := max(50, min(1870, center[0]+noiseX))
		centerY := max(50, min(1030, center[1]+noiseY))
	...
	for i := 0; i < b.N; i++ {
		...
		for j := range detections {
			for k := j + 1; k < len(detections); k++ {
				totalIoU += CalculateIoU(detections[j], detections[k])
			}
		}
...
func BenchmarkIoU_PerformanceAnalysis(b *testing.B) {
	scenarios := []struct {
		...
		{"NoOverlap", Rect{0, 0, 100, 100}, Rect{200, 200, 300, 300}},
		{"TouchingEdge", Rect{0, 0, 100, 100}, Rect{100, 0, 200, 100}},
		{"SmallOverlap", Rect{0, 0, 100, 100}, Rect{90, 90, 190, 190}},
		{"HalfOverlap", Rect{0, 0, 100, 100}, Rect{50, 50, 150, 150}},
		{"LargeOverlap", Rect{0, 0, 100, 100}, Rect{20, 20, 120, 120}},
		{"FullOverlap", Rect{50, 50, 150, 150}, Rect{50, 50, 150, 150}},
		{"LargeBoxes", Rect{0, 0, 1920, 1080}, Rect{960, 540, 1920, 1080}},
		{"TinyBoxes", Rect{10, 10, 12, 12}, Rect{11, 11, 13, 13}},
	}
	...
	for _, scenario := range scenarios {
		b.Run(scenario.name, func(b *testing.B) {
			...
			start := time.Now()
			for i := 0; i < b.N; i++ {
				_ = CalculateIoU(scenario.r1, scenario.r2)
			}
